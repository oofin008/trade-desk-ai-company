...
2. `company/decisions/LOG.md` — append-only log of company-level decisions
3. `company/BUDGET.md` — current spend status (before recommending any spend)
4. Your department's `CLAUDE.md` for role-specific context
...
- If a role matters enough to justify the cost (e.g. engineering quality), bump just that role to Opus in `roster.json` and log why in `company/decisions/LOG.md`. Don't default the whole company to Opus.
- Never invoke a more expensive model than the task needs
- Where models are set: **`adl/agents/<name>.adl.yaml` → `runtime.model`** (the source; `roster.json` and agent `.md` frontmatter are compiled from it), plus `.claude/settings.json` (global). Bump a single role by editing its spec and recompiling; log why in `company/decisions/LOG.md`. Don't default the whole company to Opus.
